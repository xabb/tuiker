...
//    };
    ...
    BootstrapTableFilter.prototype.initContainer = function() {
        var that = this;
		// Changed by Impalah
		// If btn-toolbar div present buttons does not align with other buttons in toolbar
		// Changed all references to $toolbar (i.e find and delegates) to $el (parent of buttons)
        this.$toolbar = $([
//            '<div class="btn-toolbar">',
                ...
                '<div class="btn-group btn-group-filter-refresh">',
                    '<button type="button" class="btn btn-default btn-refresh" data-toggle="dropdown">',
...
//            '</div>'
        ...
        this.$filters = this.$el.find('.btn-group-filters');

        this.$el.delegate('.btn-group-filters li', 'click', function (e) {
            e.stopImmediatePropagation();
        });

        this.$el.delegate('.btn-group-filters li .filter-enabled', 'click', function(e) {
            var $chck = $(this);
        ...
        this.$el.delegate('.btn-group-filters li :input:not(.filter-enabled)', 'click change', function(e) {
            var $inp = $(this);
        ...
        this.$el.delegate('.search-values', 'keyup', function(e) {
            var $this = $(this);
            var phrase = $this.val();
    ...
    BootstrapTableFilter.prototype.initMainButton = function() {
        this.$button = this.$el.find('.btn-filter');
    ...
    BootstrapTableFilter.prototype.initRefreshButton = function() {
        var that = this;
        this.$refreshButton = this.$el.find('.btn-refresh');
    ...
    BootstrapTableFilter.prototype.initFilters = function() {
        var that = this;
        // Modified by impalah: use i18n
        this.$buttonList.append('<li class="remove-filters"><a href="javascript:void(0)"><span class="glyphicon glyphicon-remove"></span> ' + this.options.formatRemoveFiltersMessage() + '</a></li>');
        this.$buttonList.append('<li class="divider"></li>');
        $.each(this.options.filters, function(i, filter) {
            that.addFilter(filter);
        });
        this.$el.delegate('.remove-filters *', 'click', function() {
	...
	// Modified by impalah: added filter-type
    BootstrapTableFilter.prototype.enableFilter = function(field) {
        var filter = this.getFilter(field);
        var $filterDropdown = $([
            '<div class="btn-group" data-filter-field="' + field + '" data-filter-type="' + filter.type + '">',
	...
	// Modified by impalah: include filter type
    BootstrapTableFilter.prototype.selectFilterOption = function(field, option, data) {
        ...
        filter.selectedOptions["_type"] = filter.type;
