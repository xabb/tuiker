...
!function ($) {
	...
	// Added by impalah
	/**
	 * Encode filter parameters: __filter_fieldname=value|value2 
	 * @param {Object} columns
	 * @param {Object} field
	 */
    var encodeFilterParams = function (params) {

		for(var filter in params.filter) {
			if(params.filter.hasOwnProperty(filter)) {
				// Depending on filter._type build each filter line
				var name = '_filter_';
				var values;
				
				switch(params.filter[filter]._type) {
					case 'range':
						if(params.filter[filter].hasOwnProperty('eq')) {
							name = '_filter__eq__' + filter;
					    	params[name] = params.filter[filter].eq.toString();
						}
						if(params.filter[filter].hasOwnProperty('gte')) {
							name = '_filter__gte__' + filter;
					    	params[name] = params.filter[filter].gte.toString();
						}
						if(params.filter[filter].hasOwnProperty('lte')) {
							name = '_filter__lte__' + filter;
					    	params[name] = params.filter[filter].lte.toString();
						}
						break;					
					case 'select':
					case 'ajaxSelect':
						name += '_in__' + filter;
						// Simple: get values from _values
						values = params.filter[filter]._values;
						// Patch: delete "undefined" values
						// http://stackoverflow.com/questions/281264/remove-empty-elements-from-an-array-in-javascript
						values = values.filter(function(n){ return n !== undefined; });
						// values = $.grep(values,function(n){ return(n) });
// console.dir(values);						
				    	params[name] = values.toString();
						break;					
					case 'search':
						// Most complex case
						if(params.filter[filter].hasOwnProperty('eq')) {
						...
						if(params.filter[filter].hasOwnProperty('cnt')) {
							name = '_filter__cnt__' + filter;
					    	params[name] = params.filter[filter].cnt.toString();
						}
						if(params.filter[filter].hasOwnProperty('ncnt')) {
							name = '_filter__ncnt__' + filter;
					    	params[name] = params.filter[filter].ncnt.toString();
						}
						if(params.filter[filter].hasOwnProperty('neq')) {
							name = '_filter__neq__' + filter;
					    	params[name] = params.filter[filter].neq.toString();
						}
						if(params.filter[filter].hasOwnProperty('_values')) {
							// Special filter empty, not empty
							var arrayLength = params.filter[filter]._values.length;
							for (var i = 0; i < arrayLength; i++) {
								name = '_filter__' + params.filter[filter]._values[i] +'__' + filter;
						    	params[name] = '1';
							}
		...
		// Delete filter propery from object (if exists)
    ...
    BootstrapTable.prototype.initServer = function (silent) {
        ...
        if (this.options.queryParamsType === 'limit') {
            ...
            // Encode filter params
            // Added by impalah
            params = encodeFilterParams(params);
